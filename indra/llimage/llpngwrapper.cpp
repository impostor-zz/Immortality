...
// gamma) which can't be done with the high-level interface.  The scanline
// also begins at the bottom of the image (per SecondLife conventions)
// instead of at the top, so we must assign row-pointers in "reverse" order.
