...
BOOL LLPanelPhoenix::postBuild()
{
	...
	for(LLSD::map_iterator itr = PhoenixViewerLink::phoenix_tags.beginMap(); itr != PhoenixViewerLink::phoenix_tags.endMap(); ++itr) 
	{
		tagcombo->add(itr->first,itr->second, ADD_BOTTOM, TRUE);
	}
	tagcombo->setSimple(gSavedSettings.getString("PhoenixTagColor"));
	...
	LLSliderCtrl* mShapeScaleSlider = getChild<LLSliderCtrl>("PhoenixBeamShapeScale",TRUE,FALSE);
	mShapeScaleSlider->setCommitCallback(&LLPanelPhoenix::beamUpdateCall);
	mShapeScaleSlider->setCallbackUserData(this);

	LLSliderCtrl* mBeamsPerSecondSlider = getChild<LLSliderCtrl>("PhoenixMaxBeamsPerSecond",TRUE,FALSE);
	mBeamsPerSecondSlider->setCommitCallback(&LLPanelPhoenix::beamUpdateCall);
...
void LLPanelPhoenix::refresh()
{
	...
	comboBox = getChild<LLComboBox>("PhoenixSpellBase");
	if(comboBox != NULL) 
	{
		comboBox->removeall();
		...
		comboBox->setSimple(gSavedSettings.getString("PhoenixSpellBase"));
	}
...
{
	LLPanelPhoenix* panel = (LLPanelPhoenix*)data;
	if(panel)
	{
...
void LLPanelPhoenix::onTagComboBoxCommit(LLUICtrl* ctrl, void* userdata)
{
	...
	if(box)
	{
		gSavedSettings.setString("PhoenixTagColor",box->getSimple());
		LLPrimitive::tagstring = PhoenixViewerLink::phoenix_tags[box->getSimple()].asString();
...
void LLPanelPhoenix::onSpellBaseComboBoxCommit(LLUICtrl* ctrl, void* userdata)
{
	...
	if(box)
	{
		...
		//LLPanelPhoenix* panel = (LLPanelPhoenix*)userdata;//box->getParent();
