...
// [/RLVa:KB]
		...
		// *Now* we can start the AO (settings have loaded)
		new AOInvTimer();
	...
	if(STATE_LOGIN_PROCESS_RESPONSE == LLStartUp::getStartupState())
	{
		...
		if(successful_login)
		{
			...
			if(!region_x_str.empty() && !region_y_str.empty())
			{
				...
				llinfos << "Login region: " << region_x_str << ',' << region_y_str << llendl;
			}
...
// [/RLVa:KB]
	...
	if (STATE_CLEANUP == LLStartUp::getStartupState())
	{
		...
		//KadahCoba - disable the rendering of clouds
...
void set_startup_status(const F32 frac, const std::string& string, const std::string& msg)
{
	if(gSavedSettings.getBOOL("PhoenixDisableLoginScreens"))
	{
		...
		if(new_d != last_d)
		{
			...
			LLChat chat;
			chat.mText = new_d;
			chat.mSourceType = (EChatSourceType)(CHAT_SOURCE_OBJECT+2);
